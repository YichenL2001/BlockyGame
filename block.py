...
class Block:
    ...
    def _update_children_positions(self, position: Tuple[int, int]) -> None:
        ...
        if len(self.children) != 0:
            for i in range(4):
                ...
                new_p = children_pos[i]
                child._update_children_positions(new_p)
        # first version (wrong)
        # change_x = position[0] - self.position[0]
        # change_y = position[1] - self.position[1]
        # children_pos = self._children_positions()
        # self.position = position
        # if len(self.children) != 0:
        #     for i in range(4):
        #         child = self.children[i]
        #         original_p = children_pos[i]
        #         x = original_p[0] + change_x
        #         y = original_p[1] + change_y
        #         child._update_children_positions((x, y))
    ...
    def smash(self) -> bool:
        ...
        if self.smashable():
            self.colour = None
            for i in range(4):
                new = Block(self._children_positions()[i],
                            self._child_size(), random.choice(COLOUR_LIST),
                            self.level + 1, self.max_depth)
                self.children.append(new)
            for child in self.children:
                child._smash_children()
            return True
        else:
            return False

    def _smash_children(self) -> bool:
        """Sub-divide this block so that it has four randomly generated
        children.
    ...
    def swap(self, direction: int) -> bool:
        ...
        if len(self.children) == 0:
            ...
            self.children[0]._update_children_positions((x + n, y))
            self.children[1]._update_children_positions((x, y))
            self.children[2]._update_children_positions((x, y + n))
            self.children[3]._update_children_positions((x + n, y + n))
            return True
    ...
    def combine(self) -> bool:
        ...
        c = {}
        for child in self.children:
            if child.colour in c:
                c[child.colour] += 1
            else:
                c[child.colour] = 1
        for colour in c:
            if c[colour] == 2 and len(c) == 3 or c[colour] > 2:
                self.colour = colour
                self.children = []
                return True
        return False
    ...
    def create_copy(self) -> Block:
        ...
        """
        final = Block(self.position, self.size, self.colour, self.level,
                      self.max_depth)
        if len(self.children) == 0:
            return final
        else:
            for child in self.children:
                final.children.append(child.create_copy())
            return final
